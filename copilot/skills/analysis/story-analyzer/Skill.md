...
Output:
  what_exists:
    ...
    inadequately_described:
      - "General concern about performance"
  
  what_is_unclear:
    unclear_aspects:
      - aspect: "Which pages/features are slow?"
        current_state: "Not specified"
        needed: "Specific component(s)"
      
      - aspect: "How slow is slow?"
        current_state: "Not quantified"
        needed: "Current vs. target metrics"
      
      - aspect: "Who is affected?"
        current_state: "Not specified"
        needed: "All users or specific scenario"
  ...
  completeness_assessment:
    ...
    readiness: not_ready
    effort_to_complete: high
  
  blocking_issues:
      - "Cannot start work without knowing what's slow"
      - "Cannot measure improvement without baseline"
  
  recommended_action: "Request significant clarification before proceeding"
```

### Example 3: Story with Hidden Assumptions
```yaml
Input:
  title: "Add customer email field to form"
  description: "Customers need to update their email address"
...
- Example: "Export button on customer list"

**Mostly Clear:**
- Mostly concrete, minor ambiguity
- Example: "Export to Excel (but which columns?)"

**Vague:**
- General terms, unclear specifics
...
- [ ] User role/actor clear
- [ ] Core feature defined
