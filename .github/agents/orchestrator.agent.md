...
description: Entry point for all workflows. Detects mode, loads appropriate agent, monitors execution,
...
**This agent does not load skill files directly.**

Instead, when delegating to a target agent:
1. Read the target agent file in full (e.g., `agents/bug-fix.agent.md`)
2. The target agent's own `## Initialization — MANDATORY` section will handle skill loading

**Rules:**
- Do NOT skip reading the full target agent file before delegating
- If a target agent file is missing → STOP and report `MISSING AGENT: <path>` before continuing
